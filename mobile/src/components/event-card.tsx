...
export function EventCard({ event }: { event: EventSummary }) {
    ...
    return (
        <Pressable
            ...
            style={{ backgroundColor: colors.card, borderColor: colors.border, borderWidth: 1, borderRadius: 16, padding: 17, marginBottom: 12 }}
        >
            ...
            {event.location ? (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 12 }}>
                    ...
                    <Text numberOfLines={1} style={{ color: colors.muted, fontSize: 13, flex: 1 }}>
                        {event.location}
                    </Text>
                </View>
            ) : null}
            <View style={{ borderTopColor: colors.border, borderTopWidth: 1, marginTop: 17, paddingTop: 14 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                    <View style={{ flexDirection: 'row', gap: 6 }}>
                        <Ionicons name="list-outline" size={16} color={colors.muted} />
                        <Text style={{ color: colors.muted, fontSize: 12 }}>Checklist</Text>
                    </View>
                    <Text style={{ color: progress === 100 && event.checklist_count ? '#34d399' : colors.muted, fontSize: 12 }}>
                ...
                <View style={{ height: 6, borderRadius: 6, backgroundColor: colors.border, overflow: 'hidden' }}>
                    <View style={{ height: 6, width: `${progress}%`, borderRadius: 6, backgroundColor: progress === 100 ? '#34d399' : '#3b82f6' }} />
                </View>
            </View>
        </Pressable>
    );

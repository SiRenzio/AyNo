...
export const colors = { bg: '#080b14', card: '#111827', border: '#263149', text: '#f8fafc', muted: '#94a3b8', primary: '#8b5cf6', danger: '#ef4444' };
...
export function Button({ title, onPress, danger, disabled }: { title: string; onPress: () => void; danger?: boolean; disabled?: boolean }) {
  return <Pressable disabled={disabled} onPress={onPress} style={[styles.button, danger && { backgroundColor: colors.danger }, disabled && { opacity: .55 }]}><Text style={styles.buttonText}>{title}</Text></Pressable>;
}
...
export const styles = StyleSheet.create({
  ...
  field: { color: colors.text, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, borderRadius: 12, padding: 14, marginBottom: 12 },

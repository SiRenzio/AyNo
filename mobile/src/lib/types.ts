...
export type DashboardData = { statistics: { total: number; upcoming: number; overdue: number; completed: number }; upcoming_events: EventSummary[]; completed_events: EventSummary[] };
export type EventTemplate = { id: number; name: string; description?: string; items: string[] };
export type NotificationItem = { id: number; remind_at: string; status: string; channel: string; read_at?: string | null; event: { id: number; title: string; location?: string; starts_at: string } };

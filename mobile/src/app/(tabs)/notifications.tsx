...
const filters = ['all', 'unread', 'read', 'pending', 'sent', 'completed', 'failed', 'cancelled'];
...
export default function Reminders() {
    ...
    const visible = useMemo(
        () =>
            items.filter(
                (item) => filter === 'all' || (filter === 'unread' ? !item.read_at : filter === 'read' ? !!item.read_at : item.status === filter),
            ),
        [items, filter],
    );
    ...
    function confirmBatchDelete() {
        ...
        Alert.alert('Delete selected notifications?', `${ids.length} notification${ids.length === 1 ? '' : 's'} will be permanently removed.`, [
            { text: 'Keep them', style: 'cancel' },
            {
                ...
                style: 'destructive',
                onPress: async () => {
                    await api('/notifications', { method: 'DELETE', body: JSON.stringify({ ids }) }, token);
                    exitSelection();
                    await load();
                },
            },
    ...
    function confirmUpdate(id: number, status?: 'pending' | 'cancelled') {
        ...
        Alert.alert(
            deleting ? 'Delete notification?' : cancelling ? 'Cancel reminder?' : 'Retry reminder?',
            deleting
                ? 'This notification will be permanently removed.'
                : cancelling
                  ? 'This reminder will no longer notify you.'
                  : 'This reminder will be scheduled again if its time is still in the future.',
            [
                { text: 'Not now', style: 'cancel' },
                {
                    text: deleting ? 'Delete' : cancelling ? 'Cancel reminder' : 'Retry',
                    style: deleting || cancelling ? 'destructive' : 'default',
                    onPress: () => void update(id, status),
                },
            ],
        );
    }
            ...
            <FlatList
                ...
                ListHeaderComponent={
                    <View>
                        ...
                        <Text style={[styles.subtitle, { color: colors.muted }]}>Delivered native reminders appear here automatically.</Text>
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 14 }}>
                            <Action
                                label={allVisibleSelected ? 'Clear all' : 'Check all'}
                            ...
                            {selectionMode ? (
                                <>
                                    <Action
                                        label={`Read (${selected.size})`}
                                        icon="mail-open-outline"
                                        disabled={!selected.size}
                                        onPress={() => void markRead([...selected])}
                                    />
                                    <Action
                ...
                renderItem={({ item }) => {
                    ...
                    const statusAccent = notificationStatusColors[item.status] ?? colors.muted;
                    return (
                        <Pressable
                            ...
                            style={[
                                styles.card,
                                {
                                    ...
                                    borderColor: checked || unread ? '#60a5fa' : colors.border,
                                },
                            ]}
                        >
                            <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 11 }}>
                                {selectionMode ? <SelectionBox checked={checked} /> : null}
                                <View style={{ flex: 1 }}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 10 }}>
                                        <Text numberOfLines={1} style={{ color: colors.text, fontWeight: '700', fontSize: 16, flex: 1 }}>
                                            {item.event.title}
                                        </Text>
                                        <View
                                            style={{
                                        ...
                                        >
                                            <Text style={{ color: statusAccent, fontSize: 11, fontWeight: '700', textTransform: 'capitalize' }}>
                                                {item.status}
                                            </Text>
                                        </View>
                                    </View>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 12 }}>
                                        <Ionicons name="alarm-outline" size={16} color={colors.muted} />
                                        <Text style={{ color: colors.muted, fontSize: 13 }}>Reminder: {formatDateTime(item.remind_at)}</Text>
                                    </View>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 8 }}>
                                        <Ionicons name="calendar-outline" size={16} color={colors.muted} />
                                        <Text style={{ color: colors.muted, fontSize: 13 }}>Event: {formatDateTime(item.event.starts_at)}</Text>
                                    </View>
                                    {!selectionMode ? (
                                        <View
                                            style={{
                                        ...
                                        >
                                            {unread ? (
                                                <Pressable onPress={() => void markRead([item.id])}>
                                                    <Text style={{ color: colors.primary, fontWeight: '600' }}>Mark as read</Text>
                                                </Pressable>
                                            ) : null}
                                            {item.status === 'pending' ? (
                                                <Pressable onPress={() => confirmUpdate(item.id, 'cancelled')}>
                                                    <Text style={{ color: '#d97706', fontWeight: '600' }}>Cancel</Text>
                                                </Pressable>
                                            ) : null}
                                            {['failed', 'cancelled'].includes(item.status) ? (
                                                <Pressable onPress={() => confirmUpdate(item.id, 'pending')}>
                                                    <Text style={{ color: colors.primary, fontWeight: '600' }}>Retry</Text>
                                                </Pressable>
                                            ) : null}
                                            <Pressable onPress={() => confirmUpdate(item.id)}>
    ...
    return (
        <View
            style={{
                ...
                borderRadius: 5,
                borderWidth: 1.5,
                borderColor: checked ? '#60a5fa' : '#64748b',
    ...
    return (
        <Pressable
            ...
            style={{
                ...
                backgroundColor: danger ? `${colors.danger}12` : colors.card,
                borderRadius: 9,
                paddingHorizontal: 10,
                paddingVertical: 8,
            }}
        >
            <Ionicons name={icon} size={15} color={danger ? '#fb7185' : '#93c5fd'} />
            <Text style={{ color: danger ? colors.danger : colors.text, fontSize: 12, fontWeight: '700' }}>{label}</Text>
        </Pressable>
    );
}

...
const options = (title: string, icon: keyof typeof Ionicons.glyphMap, badge = 0, badgeBorder = '#0f1422') => ({
    title,
    tabBarIcon: ({ color, size }: { color: string; size: number }) => (
        <View style={{ width: 32, height: 28, alignItems: 'center', justifyContent: 'center' }}>
            ...
            {badge ? (
                ...
                >
                    <Text style={{ color: '#fff', fontSize: 8, lineHeight: 10, fontWeight: '800', textAlign: 'center', includeFontPadding: false }}>
                        {badge > 99 ? '99+' : badge}
                    </Text>
                </View>
            ) : null}
...
export default function TabsLayout() {
    ...
    return (
        <Tabs
            screenOptions={{
                headerShown: false,
                tabBarStyle: { backgroundColor: colors.card, borderTopColor: colors.border, height: 66, paddingBottom: 8 },
                tabBarActiveTintColor: '#60a5fa',
                tabBarInactiveTintColor: '#64748b',
            }}
        >
            <Tabs.Screen name="home" options={options('Home', 'home-outline', 0, colors.card)} />
            <Tabs.Screen name="index" options={options('Events', 'calendar-outline')} />
            <Tabs.Screen name="create" options={options('Add', 'add-circle')} />
            <Tabs.Screen name="notifications" options={options('Reminders', 'notifications-outline', notificationCount, colors.card)} />
